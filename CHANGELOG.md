Release 0.3.0 - 2020-11-04
* Build with the "org.embulk.embulk-plugins" Gradle plugin [#25](https://github.com/embulk/embulk-input-ftp/pull/25)
* Catch up with Embulk v0.10.19 [#26](https://github.com/embulk/embulk-input-ftp/pull/26)

Release 0.2.1 - 2019-11-19
* Add `path_match_pattern` to filter files by regular expression [#23](https://github.com/embulk/embulk-input-ftp/pull/23)

Release 0.2.0 - 2018-12-26
* Use file name output feature in Embulk core to show file name in cmdout [#20](https://github.com/embulk/embulk-input-ftp/pull/20)

Release 0.1.6 - 2018-06-29

* Extract utility class as embulk-util-ftp

Release 0.1.5 - 2016-11-16

* Added support for Java7

Release 0.1.4 - 2016-11-16

* Added support for FTPES(FTPS explicit) 
* Added support for incremental option

Release 0.1.3 - 2015-08-18

* Upgraded embulk version to 0.7.0

Release 0.1.2 - 2015-06-10

* Added support for SSL certificate verification


Release 0.1.1 - 2015-04-29

* Added support for SSL
* Fixed last_path handling


Release 0.1.0 - 2015-04-29

* First release
